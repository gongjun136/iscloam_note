...
void loop_closure_detection()
{
    while (1)
    {
        if (!pointCloudBuf.empty() && !odometryBuf.empty())
        {
            ...
            Eigen::Quaterniond quaternion(odom_in.rotation());
            LOG(INFO) << "key frame pos: " << odom_in.translation().x() << " " << odom_in.translation().y() << " " << odom_in.translation().z()
                      << " att: " << quaternion.x() << " " << quaternion.y() << " " << quaternion.z() << " " << quaternion.w();
            ...
            for (int i = 0; i < (int)iscGeneration.matched_frame_id.size(); i++)
            {
                ...
                LOG(INFO) << "Loop found! between: " << iscGeneration.current_frame_id << "--"
                          << iscGeneration.matched_frame_id[i]
                          << ", score: 1";
            }
